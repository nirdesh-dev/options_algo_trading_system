...
#[test]
fn test_gpu_mode_attempts_cuda_initialization() {
    let domain_rules = DomainRules::default();

    let strategy_config = BollingerBandsStrategy::validate_config(
        &StrategyConfigBuilder::new()
            .periods(vec![10])
            .std_dev_factors(vec![2.0]),
        &domain_rules,
    )
    .unwrap();

    let price_data = vec![
        100.0, 101.0, 99.0, 102.0, 98.0, 103.0, 97.0, 104.0, 105.0, 106.0, 107.0,
    ];

    let backtest_engine = BacktestEngine::new(ComputeMode::GPU { cuda_device: 0 });
    let results =
        backtest_engine.run_backtest::<BollingerBandsStrategy>(&strategy_config, &price_data);

    // Should either work (if CUDA available) or fail with meaningful CUDA error
    match results {
        Ok(backtest_results) => {
            assert_eq!(backtest_results.len(), 1);
        }
        Err(e) => {
            // Should mention CUDA in error message, not generic "No implementation"
            let error_msg = e.to_string();
            assert!(
                error_msg.contains("CUDA")
                    || error_msg.contains("device")
                    || error_msg.contains("GPU"),
                "Expected CUDA-related error, got: {}",
                error_msg
            );
        }
    }
